...
class AKT(nn.Module):
    def __init__(self, n_question, stu_num, n_pid, d_model, n_blocks, dropout, d_ff=256,
        ...
        # 初始化学生记忆向量
        self.student_memory = nn.Embedding(stu_num, d_model)
        torch.nn.init.xavier_uniform_(self.student_memory.weight)
        self.default_student_vector = nn.Parameter(torch.zeros(d_model), requires_grad=False)
        ...
        self.model = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=d_model, nhead=num_attn_heads, dim_feedforward=d_ff, dropout=dropout,
                                       batch_first=True),
            num_layers=n_blocks
        )
        ...
        self.out = nn.Sequential(
            nn.Linear(d_model * 2 + d_model, final_fc_dim), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(final_fc_dim, 256), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(256, 1)
        )

    def get_student_vector(self, student_id):
        max_id = self.student_memory.num_embeddings
        if (student_id >= max_id).any():
            return self.default_student_vector.unsqueeze(0).expand(student_id.size(0), -1)
    ...
    def forward(self, q_data, target, student_id=None, pid_data=None, qtest=False):
        q_embed_data, qa_embed_data = self.base_emb(q_data, target)

        if student_id is not None:
            student_id = student_id.view(-1)
            m_s = self.get_student_vector(student_id)
            m_proj_qa = self.wkm(m_s).unsqueeze(1).expand(-1, q_embed_data.size(1), -1)
            m_proj_q = self.wrm(m_s).unsqueeze(1).expand(-1, q_embed_data.size(1), -1)
            qa_embed_data = qa_embed_data + m_proj_qa
            q_embed_data = q_embed_data + m_proj_q
        else:
            m_s = torch.zeros(q_embed_data.size(0), q_embed_data.size(2), device=q_embed_data.device)
        ...
        if self.n_pid > 0:
            ...
            if self.separate_qa:
                ...
                qa_embed_data += pid_embed_data * (qa_embed_diff_data + q_embed_diff_data)
        ...
        # Transformer 编码
        d_output = self.model(qa_embed_data)
        print("d_output.shape:", d_output.shape)
        print("q_embed_data.shape:", q_embed_data.shape)

        # 拼接个性向量（扩展）
        m_s_expanded = m_s.unsqueeze(1).expand(-1, d_output.size(1), -1)
        print("m_s_expanded.shape:", m_s_expanded.shape)
        concat_q = torch.cat([d_output, q_embed_data, m_s_expanded], dim=-1)
