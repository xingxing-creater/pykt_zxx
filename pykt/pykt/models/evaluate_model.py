    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["atdkt"]:
                ...
                y = y[:, 1:]
            elif model_name in ["extrakt","folibikt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lefokt_akt", "fluckt"]:
                y, reg_loss = model(cc.long(), cr.long(), cq.long())
                y = y[:,1:]
            elif model_name in ["akt"]:
                student_id = dcur["uid"].long().to(device)  # ✅ 添加 student_id
                y, reg_loss = model(
                    q_data=cc.long(),
                    target=cr.long(),
                    pid_data=cq.long(),
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn","skvmn"]:
                ...
                y, h = model(dcurori, qtest=True, train=False)
            elif model_name in ["extrakt", "folibikt","fluckt","lefokt_akt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
                y, reg_loss, h = model(cc.long(), cr.long(), cq.long(), True)
                y = y[:,1:]
            elif model_name in ["akt"]:
                if "uid" in dcur:
                    student_id = dcurori["uid"].long().to(device)
                else:
                    student_id = None
                y, reg_loss, h = model(cc.long(), cr.long(), cq.long(),student_id=student_id,qtest=True)
                y = y[:,1:]
            elif model_name in ["dtransformer"]:
