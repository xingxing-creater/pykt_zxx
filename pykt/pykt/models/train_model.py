...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    # elif model_name in ["akt", "extrakt", "folibikt", "robustkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn",
    #                     "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lefokt_akt",
    #                     "dtransformer", "fluckt"]:
    #     y = torch.masked_select(ys[0], sm)
    #     t = torch.masked_select(rshft, sm)
    #     loss = binary_cross_entropy(y.double(), t.double()) + preloss[0]

    elif model_name in ["extrakt", "folibikt", "robustkt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn",
                        "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lefokt_akt",
                        "dtransformer", "fluckt"]:
        y = torch.masked_select(ys[0], sm)
        t = torch.masked_select(rshft[:, 1:], sm)
        loss = binary_cross_entropy(y.double(), t.double()) + preloss[0]
    elif model_name in ["akt"]:
        y = torch.masked_select(ys[0], sm)  # ys[0] 是 y[:,1:]，已经对齐
        t = torch.masked_select(rshft, sm)  # ✅ 注意这里不要再切 rshft[:,1:]
        loss = binary_cross_entropy(y.double(), t.double()) + preloss[0]
...
def model_forward(model, data, rel=None):
    ...
    #     preloss.append(reg_loss)
        ...
        if "uid" in dcur:
            student_id = dcur["uid"].long().to(device)
        else:
            student_id = None
        ...
        y, reg_loss = model(
            ...
            #pid_data=dcur["pid_data"].long().to(device)  # ✅ 加这一行！
        )
        ys.append(y[:, 1:])
        preloss.append(reg_loss)
...
def train_model(model, train_loader, valid_loader, num_epochs, opt, ckpt_path, test_loader=None,
                test_window_loader=None, save_model=False, data_config=None, fold=None):
