...
#
#         self.input_type = input_type
#         if "uid" not in self.input_type:
#             self.input_type.append("uid")  # ✅ 自动加入 uid
#
#
#         self.qtest = qtest
...
#         """
#         dori = {"uid": [],"qseqs": [], "cseqs": [], "rseqs": [], "tseqs": [], "utseqs": [], "smasks": []}
#
...
class KTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # if 'qseqs' in dcur:
        #     qseq = dcur['qseqs']
        #     if not isinstance(qseq, torch.Tensor) or qseq.dtype != torch.long:
        #         qseq = torch.LongTensor(qseq)
        #
        #     # 保证与 input（q_data）对齐
        #     dcur['pid_data'] = qseq[:-1]  # ✅ 一定要裁掉最后一位
        #
        # print("✅✅✅✅✅✅✅✅✅✅")
        # print("DEBUG:", "qseqs in dcur?", 'qseqs' in dcur, "pid_data in dcur?", 'pid_data' in dcur,
        #       "type:", type(dcur.get('pid_data', None)), "dtype:", getattr(dcur.get('pid_data', None), "dtype", None))
    ...
    #     """
    #     自定义 collate_fn，将 list of dict 合并为 batch dict。
    #     兼容 uid + 序列型输入。
    #     """
    #     batch_dict = {}
    #     keys = batch[0].keys()
