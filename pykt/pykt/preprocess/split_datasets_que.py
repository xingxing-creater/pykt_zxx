...
def main(dname, fname, dataset_name, configf, min_seq_len = 3, maxlen = 200, kfold = 5):
    ...
    # generate sequences
    split_seqs,train_num = generate_sequences(splitdf, effective_keys, min_seq_len, maxlen)
    ...
    test_seqs, _ = generate_sequences(test_df, list(effective_keys), min_seq_len, maxlen)
    ...
    test_window_seqs,test_num = generate_window_sequences(test_df, list(effective_keys), maxlen)
    ...
    write_config(dataset_name=dataset_name, dkeyid2idx=dkeyid2idx, effective_keys=effective_keys, configf=configf,
                 dpath=dname, k=kfold, min_seq_len=min_seq_len, maxlen=maxlen, other_config=other_config,stu_num=stu_num)
